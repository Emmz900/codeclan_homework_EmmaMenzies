...
calories_analysis <- drinks_content %>% 
  select(Beverage_category, Beverage, Beverage_prep, Calories) #filter(is.na(Calories)) returns 0 rows therefore there are no NA values to remove. If there were we could perhaps use filter(na.rm = TRUE)?
calories_analysis
```
