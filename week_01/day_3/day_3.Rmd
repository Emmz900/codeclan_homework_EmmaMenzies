...
glimpse(movies)
head(movies, 10)
arrange(movies, vote_average)
```
-Consider converting the budget and revenue into a more readable format such as currency.
-Are there any differences between original_title and title?
-Look for any 'NA's
-Are any of the original languages not en?
-Could change the runtime to hours
...
#there must be a shorter way of doing something to every column??
movies_selected %>% 
  summarise(across(
    .cols = everything(),
    .fns = ~sum(is.na(.x)) # ~ to repeat function across every selected column
  ))
...
movie_budgets <- movies_imputed %>% 
  mutate(budget_type = case_when(budget < 12000000 ~ "Small budget", #could use 12e6
                                 #budget >= 12000000 & 
                                 budget < 40000000 ~ "Medium budget", # don't need the first half of this as all less than 12e6 have already been categorised
