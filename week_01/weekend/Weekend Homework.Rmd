...
The lowest rating recieved is actually 1. All 0 ratings can therefore be replaced with NA
```{r}
...
books_fixed <- books_fixed_rating %>% 
  ...
  mutate(num_pages = if_else(num_pages == 0, NA, num_pages))
...
Some titles appear to be sets despite the individual books alse being included
```{r}
books_fixed %>% 
